...
sub proc(@)         { return runProto({returnSuccess => 0, printOut => 0, printCmd => 0, pty => 0},                   @_); }
sub procChomp(@)    { return runProto({returnSuccess => 0, printOut => 0, printCmd => 0, pty => 0, chomp => 1},       @_); }
sub procUser(@)     { return runProto({returnSuccess => 0, printOut => 0, printCmd => 0, pty => 0, wrapUserCmd => 1}, @_); }
sub tryproc(@)      { return runProto({returnSuccess => 0, printOut => 0, printCmd => 0, pty => 0, fatal => 0},       @_); }
