...
sub main(){
  ...
  my $markup = ghcprinter 'TextRows', $top, $bot;
  $markup = ghcprinter 'ClickAction', 1, $lClickCmd, $markup if $lClickCmd;
  $markup = ghcprinter 'ClickAction', 2, $mClickCmd, $markup if $mClickCmd;
  $markup = ghcprinter 'ClickAction', 3, $rClickCmd, $markup if $rClickCmd;
  $markup = green $markup;
...
sub isThunderbirdRunning(){
  system "pidof $processName > /dev/null";
  return $? == 0;
}
